	...
	private:
		...
		class Key_map
		{
			...
			public:
				...
				/**
				 * Obtain modifier condition from charmap XML node
				 */
				static Key::Rule::Conditions::Modifier::Constraint
				_charmap_mod_cond(Xml_node charmap, Modifier::Name const &mod_name)
				{
					if (!charmap.has_attribute(mod_name.string()))
						return Key::Rule::Conditions::Modifier::DONT_CARE;

					bool const pressed = charmap.attribute_value(mod_name.string(), false);
				...
				void import_charmap(Xml_node charmap)
				{
					/* obtain modifier conditions from charmap attributes */
					Key::Rule::Conditions cond;
					cond.modifiers[Modifier::MOD1].constraint = _charmap_mod_cond(charmap, "mod1");
					cond.modifiers[Modifier::MOD2].constraint = _charmap_mod_cond(charmap, "mod2");
					cond.modifiers[Modifier::MOD3].constraint = _charmap_mod_cond(charmap, "mod3");
					cond.modifiers[Modifier::MOD4].constraint = _charmap_mod_cond(charmap, "mod4");

					/* add a rule for each <key> sub node */
					charmap.for_each_sub_node("key", [&] (Xml_node key_node) {
		...
		void _apply_sub_node(Xml_node const node, unsigned const max_recursion)
		{
			...
			/*
			 * Handle charmap nodes
			 */
			if (node.type() == "charmap") {
				_key_map.import_charmap(node);
				return;
			}
