                                  ...
                                  Affinity             const &affinity)
{
	...
	try {
		...
		Ram_transfer::Local_account ref_ram_account { _policy.ref_ram(), _policy.ref_ram_cap() };
		Cap_transfer::Local_account ref_cap_account { _policy.ref_pd(),  _policy.ref_pd_cap()  };

		Ram_transfer::Local_account ram_account { ram(), ram_session_cap() };
		Cap_transfer::Local_account cap_account { pd(),  pd_session_cap() };
...
void Child::_revert_quota_and_destroy(Session_state &session)
{
	...
	Ram_transfer::Local_account child_ram_account(ram(), ram_session_cap());

	Cap_transfer::Local_account   ref_cap_account(_policy.ref_pd(), _policy.ref_pd_cap());
	Cap_transfer::Account    &service_cap_account = session.service();
	Cap_transfer::Local_account child_cap_account(pd(), pd_session_cap());
